...
function M.navigate(config)
  ...
  if not (type(user_input) == "table" and #user_input == 1) then
    err("did not get one valid user input", config.debug)
    return
  end
  a = unpack(user_input)
  ts_utils.goto_node(a, false, true)
end
...
function M.iswap_node(config, direction)
  ...
  local a_range, b_range = unpack(internal.swap_nodes_and_return_new_ranges(picked_node, swap_node, bufnr, false))
